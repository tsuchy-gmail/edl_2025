...
def to_avg_dict(typ, epoch, loss, n_data, lamb, loss_in=None, loss_out=None, mse_in=None, mse_out=None, kl_in=None, kl_out=None, acc=None, acc_in=None, acc_out=None):
...
def plot_train_loss(loss_df, title, save_path, lim):
...
def test(head, loader, records, save_ok, device, lamb, epoch, result_path, epochs, min_test_loss_in, max_acc_in):
    ...
    avg_dict = to_avg_dict("test", epoch, loss_total, n_data, lamb, loss_in_total, loss_out_total, mse_in_total, mse_out_total, kl_in_total, kl_out_total, acc, acc_in, acc_out)
    ...
    if save_ok:
        ...
        plot_test_loss(test_loss_df, "Test Loss", loss_png_path, 2)
...
def train(train_loader, test_loader, epochs, cuda, save_ok, dir_suffix, crop_size, lamb_type):
    ...
    for epoch in range(epochs):
        ...
        for feat_batch, label_batch, _, case_batch in train_loader:
            feat_batch = feat_batch.to(device, non_blocking=True)
            label_batch = label_batch.to(device, non_blocking=True)
        ...
        if save_ok:
            avg_dict = to_avg_dict("train", epoch, loss_total, n_data, lamb)
            ...
            plot_train_loss(train_loss_df, "Train Loss", loss_png_path, 2)
