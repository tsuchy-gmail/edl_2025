...
def load_json(filename):
    with open(filename_patchs_count, mode="r") as f:
        return json.load(f)
...
def dump_train_test_files(train_test_files, results_dirname, side):
    ...
    with open(f"{results_dirname}/{side}/itrain_files_Reactive.json", mode="w") as f:
...
def train(excluded_cases_Reactive, excluded_cases_FL, cuda, transforms, epochs):
    torch.manual_seed(0)
...
#pair_idx: 0~8
    ...
    train([case_Reactive], [case_FL], cuda=cuda, transforms=set_transforms(), epochs=epochs)



main(pair_idx=6, cuda=0, epochs=500)
