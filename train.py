...
def test(model, loader, records, save_ok, device, lamb, epoch, result_path):
    loss_total = 0.0
    loss_in_total = 0.0
    loss_out_total = 0.0
    mse_in_total = 0.0
    ...
    with torch.no_grad():
        ...
        for img_batch, label_batch, region_batch in loader:
            ...
            n_data_in += inside_mask.sum()
            n_data_out += outside_mask.sum()
            
            #only test
            n_correct += (alpha_batch.argmax(-1) == label_batch).sum()
            n_correct_in += (alpha_batch[inside_mask].argmax(-1) == label_batch[inside_mask]).sum()
            n_correct_out += (alpha_batch[outside_mask].argmax(-1) == label_batch[outside_mask]).sum()
    ...
    print(f"n_coorect:{n_correct}\nn_correct_in:{n_correct_in}\nn_correct_out:{n_correct_out}")
    print("---")
    print(f"acc:{acc}\nacc_in:{acc_in}\nacc_out:{acc_out}")
...
def train(cuda, transform, save_dir, save_ok, ini_num_workers, num_workers):
    ...
    for epoch in range(epochs):
        ...
        avg_test_loss_in, avg_test_loss_out, acc, acc_in, acc_out = test(model, test_loader, test_records, save_ok, device, lamb, epoch, result_dir_path)
